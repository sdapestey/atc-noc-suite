...
_lock_estadisticas_altiplano = threading.Lock()
...
_estadisticas_altiplano: dict[str, dict] = {}
...
def get_cached_estadisticas_altiplano(
    ttl_seconds: int,
    token_key: str,
    factory: Callable[[], T],
) -> T:
    """Cachea conteos Altiplano INP por clave de token (servidor vs sesión)."""
    return _get_cached_dict_entry(
        _estadisticas_altiplano,
        _lock_estadisticas_altiplano,
        ttl_seconds,
        token_key,
        factory,
    )
...
def reset_dashboard_tree_caches() -> None:
    ...
    with _lock_estadisticas_altiplano:
        _estadisticas_altiplano.clear()
