...
def export_index_query_csv(value: str) -> str:
    ...
    if value.isdigit():
        res = consultar_access_id_estructura(value)
        if not res:
            w.writerow(["error", "sin datos"])
            return buf.getvalue()
        w.writerow(["AID", "OPERADOR", "Status", "CTO", "RAMA", "ONT"])
        w.writerow([res["AID"], res["OPERADOR"], res["Status"], res["CTO"], res["RAMA"], res["ONT"]])
    elif "FATC" in value:
        rows = consultar_cto_estructura(value)
        w.writerow(["AID", "OPERADOR", "PRINCIPAL", "RAMA", "ONT", "STATUS"])
        for r in rows:
            w.writerow([
                r["AID"],
                r["OPERADOR"],
        ...
        w.writerow(["CTO", "AID", "OPERADOR", "ONT", "STATUS"])
        for cto, rows in data.items():
            for r in rows:
                w.writerow([cto, r["AID"], r["OPERADOR"], r["ONT"], r["STATUS"]])
    else:
